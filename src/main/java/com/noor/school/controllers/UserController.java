...
public class UserController {
    ...
    //@RequestBody attends des données sous format json ce pour cela qu'il faut utilisé @ModelAttribute
    public User register(@RequestParam(name= "file",required = false) MultipartFile file,
                         @Valid @ModelAttribute User user, BindingResult bindingResult,@RequestParam(name = "role") String role) throws Exception {
    ...
    public User update(@RequestParam(name= "file",required = false) MultipartFile file,
                         @Valid @ModelAttribute User user, BindingResult bindingResult,@RequestParam(name = "role",required = false) String role) throws Exception {
    ...
    public String date() throws Exception{
        ...
        /* pour les date simple mais limité en terme de fonctionnalité ya oaussi la methode parse de formatter pour convertir string en date
            Date date = new Date();
    ...
    /*@RequestMapping(path = "ini")
    public void ini(){
        //NB: il faut mettre les dependences à null d'abords avant de les supprimer
        User u2 = userRepository.getOne(new Long(8));
        u2.setProfil(null);
        u2.setRoles(null);
